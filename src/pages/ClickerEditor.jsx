...
import { FONT_OPTIONS, parseLetter } from "../engine/image/letter";
import { getCached3DFont, BUNDLED_FONTS } from "../utils/fontManager";
import { useGeometryWorker } from "../hooks/useGeometryWorker";
import { getShapesFromIconify } from "../utils/iconifyLoader";
import ImageWizardModal from "../components/ImageWizardModal";
import { ControlInput, ControlSelect } from "../components/UI/EditorControls";;
...
const CROSS_TOTAL = 4.4;
const CROSS_ARM_BASE = 1.3;
    ...
    return (
        <label className="flex items-center justify-between cursor-pointer select-none">
            ...
            <div
                ...
                <span
                    className={`inline-block h-3.5 w-3.5 mt-0.5 rounded-full bg-white shadow transition-transform ${value ? "translate-x-4" : "translate-x-0.5"}`}
                />
            </div>
        </label>
    );
    ...
    return (
        <div className="rounded-2xl border border-secondary bg-white shadow-sm overflow-hidden">
            <button
                ...
                <span className="text-xs font-bold text-gray-700 flex-1 text-left">
                    {title}
                </span>
                <span
                    className={`text-secondary text-xs font-bold transition-transform duration-200 ${open ? "rotate-180" : ""}`}>
                    ▼
                </span>
            </button>
            <div
...
function ColorAssignDrawer({
    ...
    let containerClass =
        "rounded-2xl border border-secondary bg-white p-3 animate-in fade-in ";
    if (inline) {
        containerClass += "w-full mt-2 shadow-sm slide-in-from-top-2";
    } else {
        containerClass +=
            "absolute left-1/2 -translate-x-1/2 z-50 w-64 shadow-2xl ";
        if (popUp) {
    ...
    return (
        <div className={containerClass}>
                ...
                {mergedColors[brand]?.map((c) => (
                    <div
                        ...
                        <button
                            ...
                            title={c.name}
                        />
                        <span className="text-[0.5rem] text-gray-400 text-center truncate w-full font-medium leading-tight">
                            {c.name}
                        </span>
                    </div>
                ))}
            ...
            <button
                onClick={onClose}
                className="mt-2 w-full text-xs font-semibold text-white bg-secondary py-1.5 rounded-xl hover:bg-secondary/80 transition-colors">
                Close
            </button>
        </div>
    );
...
function ImageTab({
    ...
    const handleFile = useCallback(
        (file) => {
            ...
            if (
                !ok.some((t) => file.type === t) &&
                !file.name.match(/\.(png|jpg|jpeg|webp|avif|gif|svg)$/i)
            )
                return;
    ...
    return (
        <div className="space-y-2">
                ...
                {isProcessing ? (
                    ...
                    <div className="relative h-24 group">
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                            <span className="opacity-0 group-hover:opacity-100 text-white text-xs font-bold transition-opacity bg-black/60 px-3 py-1 rounded-lg">
                                Change
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-24 gap-1.5 text-gray-400 select-none">
                        ...
                        <span className="text-[10px] text-gray-400 text-center px-3">
                            PNG / JPG / SVG · transparent bg recommended
                        </span>
                    </div>
                )}
                <input
            ...
            <div className="pt-2 border-t border-secondary/10">
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wide mb-2 block">
                    Templates
                </span>
                <div className="grid grid-cols-3 gap-2">
                    {[
                        ...
                        <button
                            ...
                            onClick={async () => {
                                try {
                                    const res = await fetch(
                                        getAssetUrl(`/assets/media/images/${filename}`)
                                    );
                            ...
                            title={filename}>
                            <img
                                src={getAssetUrl(`/assets/media/images/${filename}`)}
                                alt={filename}
                ...
                <div className="flex flex-col items-center justify-center h-24 gap-1.5 text-gray-400 select-none">
                    ...
                    {fileName ? (
                        ...
                        <>
                            ...
                            <span className="text-[10px] text-gray-400 text-center px-3">
                                Paths grouped by fill → colour layers
                            </span>
                        </>
                    )}
                </div>
...
function TextTab({
    ...
    const activeFonts = allFonts.length > 0 ? allFonts : BUNDLED_FONTS;
    const fontOptions = React.useMemo(
        () =>
            activeFonts.map((f) => ({ label: f.label || f.name, value: f.id })),
        [activeFonts],
    );
...
function ColorsSection({
    ...
    const colorName = (hex) => {
        ...
        return (
            ALL_COLORS.find((c) => c.hex.toLowerCase() === hex.toLowerCase())
                ?.name ?? "Custom"
        );
    };
                    ...
                    <div className="border-t border-secondary/10 pt-2 space-y-1">
                        <div className="flex justify-between items-end mb-1">
                            <p className="text-[10px] text-gray-400">
                                Click swatch to assign filament.
                            </p>
                            <p className="text-[9px] text-secondary font-semibold">
                                Select 🖌️ to paint 3D parts
                            </p>
                        </div>
                        <div className="grid grid-cols-4 gap-1.5">
                            {colorLayers.map((layer) => (
                                <div
                                    key={layer.id}
                                    className="relative flex flex-col items-center gap-1 bg-gray-50 p-1.5 rounded-xl border border-gray-100 hover:border-secondary/30 transition-colors">
                                    <button
                                        onClick={() =>
                                            setOpenDrawer(
                                                openDrawer === layer.id
                                        ...
                                        title={layer.name}
                                    />
                                    <span className="text-[0.5rem] text-gray-500 text-center truncate w-full font-medium">
                                        {layer.name}
                                    </span>
                                    <button
                                        onClick={() =>
                                            setActivePaintLayerId(layer.id)
                                        }
                        ...
                        {colorLayers.map(
                            (layer) =>
                                layer.id === openDrawer && (
                                    <ColorAssignDrawer
                                        key={`drawer-${layer.id}`}
                                        currentHex={layer.hex}
                                        onSelect={(c) =>
                                            onLayerColorChange(layer.id, c)
                                        }
                                        isOpen={true}
                            ...
                            {colorLayers.map((layer) => (
                                <div key={layer.id} className="space-y-0.5">
                                    <div className="flex items-center gap-1.5">
                                        <div
                                            className="w-3.5 h-3.5 rounded-full flex-shrink-0 border border-gray-200"
                                            style={{
                ...
                {[
                    {
                        ...
                        label: "Shell & Stem",
                        hex: shellColor,
                        setHex: setShellColor,
                    },
                    {
                        key: "capBase",
                        label: "Cap Base",
                        hex: capBaseColor,
                        setHex: setCapBaseColor,
                    },
        ...
        <AccordionSection number="2" title="More Settings">
            ...
            <div className="space-y-1.5">
                ...
                <select
                    ...
                    <option value="outline">Custom (Image Outline)</option>
            ...
            <div className="border-t border-secondary/10 pt-2 space-y-2">
                ...
                <EdgeSelect
                    label="Cap Top"
                    type={edgeCapTop}
                    onTypeChange={setEdgeCapTop}
                    radius={edgeCapTopRadius}
                    onRadiusChange={setEdgeCapTopRadius}
                />
                <EdgeSelect
                    label="Base"
                    type={edgeBase}
                    onTypeChange={setEdgeBase}
                    radius={edgeBaseRadius}
                    onRadiusChange={setEdgeBaseRadius}
                />
            </div>
            ...
            <div className="border-t border-secondary/10 pt-2 space-y-2">
                ...
                <Slider
                    label="Socket cavity delta"
                    value={switchSocketTol}
                    min={-0.5}
                    max={1.0}
                    step={0.05}
                    onChange={setSwitchSocketTol}
                />
                <Slider
                    label="Stem arm delta"
                    value={switchStemTol}
                    min={-0.3}
                    max={0.3}
                    step={0.05}
                    onChange={setSwitchStemTol}
                />
            </div>
            ...
            <div className="border-t border-secondary/10 pt-2 space-y-2">
                <Toggle
                    label="Keychain Hole"
                    value={keychainHole}
                    onChange={setKeychainHole}
                />
                {keychainHole && (
                    <>
                        <Slider
                            label="Hole Diameter"
                            value={keychainHoleDiam}
                            min={2}
                            max={8}
                            step={0.5}
                            onChange={setKeychainHoleDiam}
                        />
                        <div className="space-y-1">
                            <span className="text-xs font-semibold text-gray-700">
                                Position
                            ...
                            <SegmentedControl
                                value={keychainPosition}
                                onChange={setKeychainPosition}
                                options={[
                                    { value: "top", label: "Top" },
                        ...
                        <Slider
                            label="Slide Offset"
                            value={keychainSlideOffset}
                            min={-8}
                            max={8}
                            step={0.5}
                            onChange={setKeychainSlideOffset}
                        />
                    </>
                )}
            </div>
    ...
    return (
        <AccordionSection
            ...
            {Array.from({ length: switchCount }).map((_, i) => (
                <div
                    key={i}
                    className="border-t border-secondary/10 pt-2 space-y-1.5">
                    <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wide">
                        Switch {i + 1} offset
                    </span>
                    <div className="grid grid-cols-2 gap-2">
                        <Slider
                            label="X offset"
                            value={switchOffsets[i]?.x ?? 0}
                            min={-5}
                            max={5}
                            step={0.5}
                            onChange={(v) => updateOffset(i, "x", v)}
                        />
                        <Slider
                            label="Y offset"
                            value={switchOffsets[i]?.y ?? 0}
                            min={-5}
                            max={5}
                            step={0.5}
                            onChange={(v) => updateOffset(i, "y", v)}
                        />
                    </div>
                    <Slider
                        label="Rotation"
                        value={switchOffsets[i]?.rotation ?? 0}
                        min={-45}
                        max={45}
                        step={5}
                        onChange={(v) => updateOffset(i, "rotation", v)}
                        unit="°"
                    />
                </div>
            ))}
...
function ClickerSidebar(props) {
    const { onBack, onSave, onLoad, importMode, setImportMode, ...rest } =
        props;
        ...
        <div className="flex flex-col text-neutral space-y-2 pb-4">
            ...
            <div className="flex-shrink-0 flex items-center justify-between">
                ...
                <div className="flex gap-1">
                    <button
                        onClick={onSave}
                        title="Save Project"
                        className="flex items-center gap-1 text-xs font-semibold text-secondary border border-secondary/40 rounded-lg px-2 py-1 hover:bg-secondary/10 transition-colors">
                        💾 Save
                    </button>
                    <label
                        title="Load Project"
                        className="flex items-center gap-1 text-xs font-semibold text-gray-600 border border-gray-300 rounded-lg px-2 py-1 hover:bg-gray-50 transition-colors cursor-pointer">
                        📂 Load
                        <input
                            type="file"
                            accept=".kcp,.json"
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-2.5 space-y-0.5">
                ...
                <div className="text-[9px] text-gray-400 space-y-0.5 font-medium">
                    ...
                    <p>
                        Stem: Ø {(STEM_OUTER_R * 2).toFixed(1)} mm ·{" "}
                        {STEM_LENGTH} mm deep
                    </p>
                    <p>
                        Cross: {CROSS_TOTAL} × {CROSS_TOTAL} mm · arm{" "}
                        {CROSS_ARM_BASE} mm
                    </p>
                </div>
            </div>
    ...
    const geo = useMemo(() => {
        ...
        if (meshData.numProp === 3) {
            ...
            for (let i = 0; i < count; i++) {
                positions[i * 3] =
                    meshData.vertProperties[i * meshData.numProp];
                positions[i * 3 + 1] =
                    meshData.vertProperties[i * meshData.numProp + 1];
                positions[i * 3 + 2] =
                    meshData.vertProperties[i * meshData.numProp + 2];
            }
        }
    ...
    const geo = useMemo(() => {
        ...
        if (part.numProp === 3) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                positions[i * 3 + 1] =
                    part.vertProperties[i * part.numProp + 1];
                positions[i * 3 + 2] =
                    part.vertProperties[i * part.numProp + 2];
            }
        }
        geometry.setAttribute(
            "position",
    ...
    return (
        <mesh
            ...
            onClick={(e) => {
                e.stopPropagation();
                if (part.name) {
                    const event = new CustomEvent("partclick", {
                        detail: part.name,
                    });
...
function ClickerModel({
        ...
        if (regionSet?.regions) {
            ...
            regionSet.regions.forEach((r, i) => {
                r.components.forEach((comp, j) => {
                    ...
                    const targetLayerId =
                        partOverrides[partName] || `layer_${i}`;
                    const layer =
                        colorLayers?.find((l) => l.id === targetLayerId) ||
                        colorLayers?.[i];
                    const rgb = layer?.hex
                        ? [
            ...
            regionSet.colorLayers.forEach((l, i) => {
                ...
                l.shapes?.forEach((shapeOrRings, j) => {
                    ...
                    if (
                        shapeOrRings &&
                        typeof shapeOrRings.getPoints === "function"
                    ) {
                        ...
                        const size = buildParams?.capSize
                            ? buildParams.capSize * 0.8
                            : 20;
                        const outer = shapeOrRings
                            .getPoints(16)
    ...
    const {
        ...
        switchPlacements,
        isBuilding,
        warnings: geomWarnings,
        switchMesh,
    } = useGeometryWorker(
        getAssetUrl("/switch/mx/mx-socket.3mf"),
        getAssetUrl("/switch/mx/mx-stem.3mf"),
        getAssetUrl("/switch/mx/mx-switch.3mf"),
        workerRegions,
        workerOutline,
        buildParams,
    );
    ...
    const targetCapOffset =
        viewMode === "exploded" ? buildParams?.imageDepth * 1.5 + 10 : 0;

    useFrame((_, delta) => {
        if (topGroupRef.current) {
            topGroupRef.current.position.z +=
                (targetCapOffset - topGroupRef.current.position.z) *
                Math.min(1, delta * 5);
        }
    });
    ...
    const modelOffset = React.useMemo(() => {
        if (!parts || parts.length === 0)
            return { x: 0, y: 0, z: 0, bounds: { x: 0, y: 0, z: 0 } };
        let minX = Infinity,
            minY = Infinity,
            minZ = Infinity;
        let maxX = -Infinity,
    ...
    return (
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
            <button
                onClick={() =>
                    setViewMode((m) =>
                        m === "assembled" ? "exploded" : "assembled",
                    )
                }
                        ...
                        <button
                            onClick={() =>
                                onLayerLiftChange(
                                    ...
                                    Math.max(
                                        0,
                                        +(sel.liftHeight - 0.25).toFixed(2),
                                    ),
                                )
                            }
                            className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-secondary/20 text-gray-700 font-bold text-sm transition-colors flex items-center justify-center">
                            −
                        </button>
                        <span className="text-sm font-bold text-secondary tabular-nums w-12 text-center">
                            {sel.liftHeight}mm
                        </span>
                        <button
                            onClick={() =>
...
const CLICKER_CAM_POS = [60, 65, 90];
...
export default function ClickerEditor() {
    ...
    const [historyState, setHistoryState, { undo, redo, reset, canUndo, canRedo }] = useHistoryState({
        importMode: CLICKER_DEFAULTS.importMode,
        colorCount: CLICKER_DEFAULTS.colorCount,
        smoothing: CLICKER_DEFAULTS.smoothing,
        removeBackground: CLICKER_DEFAULTS.removeBackground,
        iconId: CLICKER_DEFAULTS.iconId,
        textContent: CLICKER_DEFAULTS.textContent,
        textFont: CLICKER_DEFAULTS.textFont,
        baseShape: CLICKER_DEFAULTS.baseShape,
        capSize: CLICKER_DEFAULTS.capSize,
        capDepth: CLICKER_DEFAULTS.capDepth,
        imageDepth: CLICKER_DEFAULTS.imageDepth,
        filletRadius: CLICKER_DEFAULTS.filletRadius,
        edgeCapTop: CLICKER_DEFAULTS.edgeCapTop,
        edgeCapTopRadius: CLICKER_DEFAULTS.edgeCapTopRadius,
        edgeBase: CLICKER_DEFAULTS.edgeBase,
        edgeBaseRadius: CLICKER_DEFAULTS.edgeBaseRadius,
        switchSocketTol: CLICKER_DEFAULTS.switchSocketTol,
        switchStemTol: CLICKER_DEFAULTS.switchStemTol,
        keychainHole: CLICKER_DEFAULTS.keychainHole,
        keychainHoleDiam: CLICKER_DEFAULTS.keychainHoleDiam,
        keychainPosition: CLICKER_DEFAULTS.keychainPosition,
        keychainSlideOffset: CLICKER_DEFAULTS.keychainSlideOffset,
        switchCount: CLICKER_DEFAULTS.switchCount,
        switchOffsets: CLICKER_DEFAULTS.switchOffsets,
        shellColor: CLICKER_DEFAULTS.shellColor,
        capBaseColor: CLICKER_DEFAULTS.capBaseColor,
        viewMode: CLICKER_DEFAULTS.viewMode,
        showMXSwitch: CLICKER_DEFAULTS.showMXSwitch,
    });

    const { importMode, colorCount, smoothing, removeBackground, iconId, textContent, textFont, baseShape, capSize, capDepth, imageDepth, filletRadius, edgeCapTop, edgeCapTopRadius, edgeBase, edgeBaseRadius, switchSocketTol, switchStemTol, keychainHole, keychainHoleDiam, keychainPosition, keychainSlideOffset, switchCount, switchOffsets, shellColor, capBaseColor, viewMode, showMXSwitch } = historyState;

    const setImportMode = (val) => setHistoryState(p => ({...p, importMode: typeof val === 'function' ? val(p.importMode) : val}));
    const setColorCountRaw = (val) => setHistoryState(p => ({...p, colorCount: typeof val === 'function' ? val(p.colorCount) : val}));
    const setSmoothing = (val) => setHistoryState(p => ({...p, smoothing: typeof val === 'function' ? val(p.smoothing) : val}));
    const setRemoveBackground = (val) => setHistoryState(p => ({...p, removeBackground: typeof val === 'function' ? val(p.removeBackground) : val}));
    const setIconId = (val) => setHistoryState(p => ({...p, iconId: typeof val === 'function' ? val(p.iconId) : val}));
    const setTextContent = (val) => setHistoryState(p => ({...p, textContent: typeof val === 'function' ? val(p.textContent) : val}));
    const setTextFont = (val) => setHistoryState(p => ({...p, textFont: typeof val === 'function' ? val(p.textFont) : val}));
    const setBaseShape = (val) => setHistoryState(p => ({...p, baseShape: typeof val === 'function' ? val(p.baseShape) : val}));
    const setCapSize = (val) => setHistoryState(p => ({...p, capSize: typeof val === 'function' ? val(p.capSize) : val}));
    const setCapDepth = (val) => setHistoryState(p => ({...p, capDepth: typeof val === 'function' ? val(p.capDepth) : val}));
    const setImageDepth = (val) => setHistoryState(p => ({...p, imageDepth: typeof val === 'function' ? val(p.imageDepth) : val}));
    const setFilletRadius = (val) => setHistoryState(p => ({...p, filletRadius: typeof val === 'function' ? val(p.filletRadius) : val}));
    const setEdgeCapTop = (val) => setHistoryState(p => ({...p, edgeCapTop: typeof val === 'function' ? val(p.edgeCapTop) : val}));
    const setEdgeCapTopRadius = (val) => setHistoryState(p => ({...p, edgeCapTopRadius: typeof val === 'function' ? val(p.edgeCapTopRadius) : val}));
    const setEdgeBase = (val) => setHistoryState(p => ({...p, edgeBase: typeof val === 'function' ? val(p.edgeBase) : val}));
    const setEdgeBaseRadius = (val) => setHistoryState(p => ({...p, edgeBaseRadius: typeof val === 'function' ? val(p.edgeBaseRadius) : val}));
    const setSwitchSocketTol = (val) => setHistoryState(p => ({...p, switchSocketTol: typeof val === 'function' ? val(p.switchSocketTol) : val}));
    const setSwitchStemTol = (val) => setHistoryState(p => ({...p, switchStemTol: typeof val === 'function' ? val(p.switchStemTol) : val}));
    const setKeychainHole = (val) => setHistoryState(p => ({...p, keychainHole: typeof val === 'function' ? val(p.keychainHole) : val}));
    const setKeychainHoleDiam = (val) => setHistoryState(p => ({...p, keychainHoleDiam: typeof val === 'function' ? val(p.keychainHoleDiam) : val}));
    const setKeychainPosition = (val) => setHistoryState(p => ({...p, keychainPosition: typeof val === 'function' ? val(p.keychainPosition) : val}));
    const setKeychainSlideOffset = (val) => setHistoryState(p => ({...p, keychainSlideOffset: typeof val === 'function' ? val(p.keychainSlideOffset) : val}));
    const setSwitchCount = (val) => setHistoryState(p => ({...p, switchCount: typeof val === 'function' ? val(p.switchCount) : val}));
    const setSwitchOffsets = (val) => setHistoryState(p => ({...p, switchOffsets: typeof val === 'function' ? val(p.switchOffsets) : val}));
    const setShellColor = (val) => setHistoryState(p => ({...p, shellColor: typeof val === 'function' ? val(p.shellColor) : val}));
    const setCapBaseColor = (val) => setHistoryState(p => ({...p, capBaseColor: typeof val === 'function' ? val(p.capBaseColor) : val}));
    const setViewMode = (val) => setHistoryState(p => ({...p, viewMode: typeof val === 'function' ? val(p.viewMode) : val}));
    const setShowMXSwitch = (val) => setHistoryState(p => ({...p, showMXSwitch: typeof val === 'function' ? val(p.showMXSwitch) : val}));
    ...
    const [customOutlineClearanceShape, setCustomOutlineClearanceShape] =
        useState(null);
    const [customOutlineOuterShape, setCustomOutlineOuterShape] =
        useState(null);
    ...
    const buildParams = useMemo(() => {
        ...
        return {
            baseShape,
            capWidthMm: capSize,
            topThickness: capDepth - imageDepth,
            imageDepth,
            imageMargin: 1,
            borderWidth: 1.5,
            capProud: 1.5,
            imageOffset: { x: 0, y: 0 },
            tolerance: switchSocketTol,
            stemTolerance: switchStemTol,
            colorBleed: 0.1,
            stepHeight: 0.25,
            travel: 3.5,
            floorThickness: 1.6,
            switches: switchOffsets
                .slice(0, switchCount)
                .map((o) => ({
                    x: o.x ?? 0,
                    y: o.y ?? 0,
            ...
            keychain: {
                enabled: keychainHole,
                style: "loop",
                angleDeg:
                    { top: 90, right: 0, bottom: 270, left: 180 }[
                        keychainPosition
                    ] ?? 90,
                holeDiameterMm: keychainHoleDiam,
                offsetMm: keychainSlideOffset,
            },
    ...
    const onLayerColorChange = useCallback((id, colorEntry) => {
        setColorLayers((prev) =>
            prev.map((l) =>
                l.id === id
                    ? { ...l, hex: colorEntry.hex, name: colorEntry.name }
                    : l,
            ),
        );
    }, []);
    ...
    const onLayerLiftChange = useCallback((id, liftHeight) => {
        setColorLayers((prev) =>
            prev.map((l) => (l.id === id ? { ...l, liftHeight } : l)),
        );
    }, []);
    ...
    const setColorCount = useCallback(
        (n) => {
            setColorCountRaw(n);
            if (adjustedImgData && importMode === "image")
                runProcessing(adjustedImgData, n);
        },
    ...
    useEffect(() => {
        ...
        const t = setTimeout(
            () => runProcessing(adjustedImgData, colorCount),
            400,
        );
    ...
    // ── Text processing ───────────────────────────────────────────────────────
    const runTextProcessing = useCallback(
        async (text, fontId) => {
            if (!text?.trim()) {
                setColorLayers([]);
                setRegionSet(null);
            ...
            try {
                const activeFonts =
                    allFonts.length > 0 ? allFonts : BUNDLED_FONTS;
                const fontOption =
                    activeFonts.find((f) => String(f.id) === String(fontId)) ||
                    activeFonts[0];
                if (!fontOption) throw new Error("No font available.");

                const font = await getCached3DFont(fontOption);

                const result = parseLetter(text, font, 15, false);
                const layers = result.regions.map((r, i) => {
                    const hex =
                        "#" +
                        r.quantRgb
                            .map((c) =>
    ...
    const handleTextChange = useCallback(
        (text) => {
            ...
            textDebounce.current = setTimeout(
                () => runTextProcessing(text, textFont),
                600,
            );
        },
    ...
    // Guard: custom shape needs an image, SVG, or text
    useEffect(() => {
        if (
            baseShape === "outline" &&
            importMode !== "image" &&
    ...
    // ── Dimension overlay ─────────────────────────────────────────────────────
    const dims = actualDims || {
        x: (capSize + 6).toFixed(1),
        y: (capSize + 6).toFixed(1),
        z: "...",
    };
    ...
    const handleSave = useCallback(() => {
        const state = {
            ...
            viewMode,
            showMXSwitch,
            colorLayers: colorLayers.map(({ id, hex, name, liftHeight }) => ({
                id,
                hex,
                name,
                liftHeight,
        ...
        a.href = url;
        a.download = "clicker_project.kcp";
    ...
    const handleLoad = useCallback((jsonStr) => {
        try {
            ...
            if (s.removeBackground !== undefined)
            ...
            if (s.switchSocketTol !== undefined)
                setSwitchSocketTol(s.switchSocketTol);
            if (s.switchStemTol !== undefined)
                setSwitchStemTol(s.switchStemTol);
            ...
            if (s.keychainSlideOffset !== undefined)
                setKeychainSlideOffset(s.keychainSlideOffset);
            ...
            if (Array.isArray(s.colorLayers))
                setColorLayers(
                    s.colorLayers.map((l) => ({ ...l, shapes: [] })),
                );
        } catch (err) {
