...
import { FONT_OPTIONS } from "./KeychainEditor";
            ...
            <div className="flex gap-10 mb-4 border-b pb-2 border-secondary">
                {Object.keys(colors).map((brand) => (
                    <button
                        ...
                        className={`text-sm font-bold ${selectedBrand === brand ? "text-primary border-b-2 border-primary " : "text-gray-400"}`}>
...
export default function CharmsEditor() {
    ...
    const activePresetIcons = ICON_PRESETS.find(p => p.label === presetTab)?.icons || [];
    ...
    const selectedFont = FONT_OPTIONS.find(f => f.id === activeCharm?.font) || FONT_OPTIONS[0];
    ...
    return (
                            ...
                            {charms.map((c, i) => (
                                ...
                                >
                                    <span className="text-sm font-semibold text-gray-700 capitalize">
                                        {i + 1}. {c.type === "icon" ? c.iconId.split(":").pop().replace(/-/g, " ") : `Text: "${c.text}"`}
                                    </span>
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                        ...
                        <>
                            ...
                            <div className="flex bg-gray-100 p-1 rounded-xl w-full border border-gray-200">
                                {["icon", "text"].map((t) => (
                                    <button
                                        key={t}
                                        onClick={() => updateActiveCharm({ type: t })}
                                ...
                                <div className="grid gap-1.5 relative">
                                    ...
                                    <button
                                        ...
                                        <span className="flex items-center gap-2 capitalize">
                                            😊 Icon Library ({activeCharm.iconId.split(":").pop()})
                                        </span>
                                    ...
                                    {isIconOpen && (
                                        <div className="absolute left-0 right-0 top-[calc(100%+0.35rem)] z-30 rounded-2xl border border-secondary bg-white p-3 shadow-2xl">
                                            ...
                                            <div className="grid grid-cols-5 gap-2 max-h-56 overflow-y-auto p-1 scrollbar-thin">
                                                {iconLoading ? (
                                                    <div className="col-span-5 text-center text-xs text-gray-400 py-4">Searching...</div>
                                                ) : displayIcons.length === 0 ? (
                                                    <div className="col-span-5 text-center text-xs text-gray-400 py-4">No icons found.</div>
                                                ) : (
                                                    displayIcons.map((iconId) => {
                                                        const [prefix, name] = iconId.split(":");
                                                        const url = `https://api.iconify.design/${prefix}/${name}.svg`;
                                                        return (
                                                            <div key={iconId} className="relative group aspect-square p-2 rounded-lg border flex items-center justify-center transition-colors bg-white hover:border-secondary">
                                                                <button
                                                                    onClick={() => {
                                                                        updateActiveCharm({ iconId });
                                                                    }}
                                                                    className={`w-full h-full flex items-center justify-center ${
                                                                        activeCharm.iconId === iconId ? "text-secondary" : "text-gray-600 group-hover:text-secondary"
                                                                    }`}
                                                                    title={iconId}
                                                                >
                                                                    <img src={url} alt={iconId} className="w-full h-full object-contain pointer-events-none opacity-80" />
                                                                </button>
                                                                <button
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                                        const newId = `charm_${Date.now()}_${Math.random()}`;
                                                                        setCharms(prev => [...prev, { ...CHARMS_DEFAULTS, id: newId, type: "icon", iconId }]);
                                                                        setSelectedCharmId(newId);
                                                                    }}
                                            ...
                                            <div className="text-[10px] text-gray-400 text-center mt-3 border-t pt-2 border-gray-100">
                                                Powered by <strong className="text-gray-500">Iconify</strong> (200k+ icons).
                                            </div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="rounded-2xl border border-secondary bg-white p-3 shadow-sm space-y-3">
                                    <label className="grid gap-1.5">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => updateActiveCharm({ text: e.target.value })}
                                                ...
                                                <div className="max-h-44 overflow-y-auto rounded-xl border border-secondary bg-gray-50 p-1">
                                                    ...
                                                    {filteredFonts.map((font) => (
                                                        <button
                                                            ...
                                                            type="button"
                                                            onClick={() => {
                                                                updateActiveCharm({ font: font.id });
                                                                setIsFontOpen(false);
                                                                setFontSearch("");
                                                            }}
                                                            className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                                                                font.id === activeCharm.font
                            ...
                            <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <Slider
                                        ...
                                        max={50}
                                        step={1}
                                        onChange={(val) => updateActiveCharm({ size: val })}
                                    />
                                    <Slider
                                        label="Charm Depth"
                                        ...
                                        max={10}
                                        step={1}
                                        onChange={(val) => updateActiveCharm({ depth: val })}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm space-y-3">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-semibold text-gray-700">Bubble Style</span>
                                    </div>
                                    <button
                                        onClick={() => updateActiveCharm({ bubbleMode: !activeCharm.bubbleMode })}
                                        className={`relative w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none ${
                                            activeCharm.bubbleMode ? "bg-secondary" : "bg-gray-300"
                                        }`}
                                    >
                                        <span
                                            className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${
                                                activeCharm.bubbleMode ? "translate-x-5" : "translate-x-0"
                                            }`}
                                        />
                                    </button>
                                </div>
                                {activeCharm.bubbleMode && (
                                    <Slider
                                        ...
                                        max={5}
                                        step={0.5}
                                        onChange={(val) => updateActiveCharm({ bubbleSize: val })}
                                    />
                                )}
                            </div>
                            ...
                            <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <Slider
                                        ...
                                        max={8}
                                        step={1}
                                        onChange={(val) => updateActiveCharm({ baseOffset: val })}
                                    />
                                    <Slider
                                        label="Base Height"
                                        ...
                                        max={12}
                                        step={1}
                                        onChange={(val) => updateActiveCharm({ baseHeight: val })}
                                    />
                                </div>
                                <div className="grid grid-cols-2 gap-3 mt-3">
                                    <Slider
                                        ...
                                        max={2}
                                        step={1}
                                        onChange={(val) => updateActiveCharm({ holeCount: val })}
                                    />
                                    <Slider
                                        label="Hole Size (mm)"
                                        ...
                                        max={10}
                                        step={0.5}
                                        onChange={(val) => updateActiveCharm({ holeDiameter: val })}
                                    />
                                </div>
                                {activeCharm.holeCount === 2 && (
                                    <div className="mt-3">
                                        <Slider
                                            ...
                                            value={activeCharm.holeDistance ?? 4}
                                            min={-10}
                                            max={30}
                                            step={1}
                                            onChange={(val) => updateActiveCharm({ holeDistance: val })}
                                        />
                                    </div>
                                )}
                                <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100">
                                    <span className="text-xs font-semibold text-gray-700">Hole Orientation</span>
                                    <div className="flex bg-gray-100 p-1 rounded-xl w-full border border-gray-200">
                                        {["horizontal", "front-to-back"].map((dir) => (
                                            <button
                                                key={dir}
                                                onClick={() => updateActiveCharm({ holeOrientation: dir })}
                                                className={`flex-1 text-xs font-bold py-1.5 rounded-lg capitalize transition-all ${
                                                    activeCharm.holeOrientation === dir
                                                        ? "bg-white text-secondary shadow-sm"
                                                        : "text-gray-400 hover:text-gray-600"
                                                }`}
                                            >
                                                {dir === "horizontal" ? "Horizontal (L/R)" : "Front-to-Back"}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm space-y-3 relative">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <label className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-gray-700">Base Color</span>
                                        <button
                                            onClick={() => {
                                                setIsBaseColorOpen(!isBaseColorOpen);
                                        ...
                                        >
                                            <div
                                                ...
                                                style={{ backgroundColor: activeCharm.baseColor }}
                                            />
                                            <span className="text-xs font-semibold text-neutral">
                                                {getColorName(activeCharm.baseColor)}
                                            </span>
                                        </button>
                                    </label>
                                    <ColorSelectionDrawer
                                        value={activeCharm.baseColor}
                                        onChange={(c) => updateActiveCharm({ baseColor: c })}
                                        isOpen={isBaseColorOpen}
                                        onClose={() => setIsBaseColorOpen(false)}
                                    />

                                    {/* Charm Color Picker */}
                                    <label className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-gray-700">Charm Color</span>
                                        <button
                                            onClick={() => {
                                                setIsCharmColorOpen(!isCharmColorOpen);
                                        ...
                                        >
                                            <div
                                                className="h-4 w-4 rounded-full border border-gray-300"
                                                style={{ backgroundColor: activeCharm.charmColor }}
                                            />
                                            <span className="text-xs font-semibold text-neutral">
                                                {getColorName(activeCharm.charmColor)}
                                            </span>
                                        </button>
                                    </label>
                                    <ColorSelectionDrawer
                                        value={activeCharm.charmColor}
                                        onChange={(c) => updateActiveCharm({ charmColor: c })}
                                        isOpen={isCharmColorOpen}
                                        onClose={() => setIsCharmColorOpen(false)}
                                    />
                                </div>
                            </div>
                        </>
        ...
        >
            <EditorCanvas
                title="KenCreations_Charms"
                dims={dims}
            >
                {charmsWithLayout.map((charm) => (
                    <group key={charm.id} position={[charm.layoutX, charm.layoutY, charm.layoutZ]}>
