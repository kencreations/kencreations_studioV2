...
function KeychainSidebar({
    ...
    // Merge predefined fonts with custom fonts
    const allFonts = useMemo(() => {
        return [...FONT_OPTIONS, ...(window.customFonts || [])];
    }, [window.customFonts]);
                    ...
                    <div className="relative">
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex flex-1 items-center justify-between rounded-2xl border border-secondary bg-gray-50 px-4 py-2.5 text-left text-xs text-neutral hover:border-secondary focus:outline-none transition-colors">
                                <span className="truncate mr-2">{selectedFont?.label}</span>
                                <span className="text-primary text-xs">
                                    {isFontOpen ? "▲" : "▼"}
                                </span>
                            </button>
                            {window.electronAPI && (
                                <label className="cursor-pointer flex items-center justify-center bg-gray-100 hover:bg-gray-200 border border-secondary rounded-2xl px-3 text-xs font-bold text-gray-700 transition-colors">
                                    +
                                ...
                                <div className="max-h-44 overflow-y-auto rounded-xl border border-secondary bg-gray-50 p-1">
                                    ...
                                    {filteredFonts.map((font) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                onFontChange(font.id); // ← Bug 1 fix: was calling undefined onFontChange
                                                setIsFontOpen(false);
                                                setFontSearch("");
                                            }}
...
export default function KeychainEditor() {
    ...
    // Load custom fonts on mount if in Electron
    useEffect(() => {
        const loadFonts = async () => {
            if (window.electronAPI) {
                const loaded = await window.electronAPI.loadCustomFonts();
                window.customFonts = loaded.map(f => ({
                    id: f.name.replace(/\.[^/.]+$/, ""),
            ...
            if (!fontCacheRef.current[selectedFontId]) {
                ...
                const parsed = await new Promise((res, rej) => {
                    if (fontOption.isCustom) {
                        // Parse custom font from buffer directly
                        try {
                            const fontJson = ttfLoader.parse(fontOption.buffer.buffer);
                            res(fontLoader.parse(fontJson));
                        } catch (err) {
                            rej(err);
                        }
                    } else {
                        // Load normal font by URL
                        ttfLoader.load(
                            fontOption.url,
